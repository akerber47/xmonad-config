...
--
myWorkspaces    = map show [1..15]
 
-- Border colors for unfocused and focused windows, respectively.
--
myNormalBorderColor  = "#666666"
-- myFocusedBorderColor = "#5D8A8A"
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    ...
    -- launch xlock
    ((modMask .|. shiftMask, xK_x	), spawn "fortune -s -n 80 | fmt -w 30 > ~/my_fortune; xlock")
    -- xlock, consultant style
    , ((modMask .|. shiftMask, xK_c	), spawn "echo 'The consultant will\nbe back shortly' > ~/my_fortune; xlock")
...
--      , ((modMask .|. shiftMask, xK_Left),  shiftToPrev >> prevWS)
    -- changed to htns for consistency w/ vim & vimperator
    ...
    , ((modMask,               xK_s), moveTo Next NonEmptyWS)
    , ((modMask .|. shiftMask, xK_s), shiftToNext >> nextWS)
    ...
    , ((modMask,               xK_r     ), sendMessage Expand)
 
    -- Push window back into tiling
    , ((modMask .|. shiftMask, xK_p     ), withFocused $ windows . W.sink)
 
    -- Increment the number of windows in the master area
    , ((modMask .|. shiftMask, xK_Home ), sendMessage (IncMasterN 1))
